...
import { acceptRequestAction, declineRequestAction } from "@/app/app/actions";

type Res = { ok: true } | { ok: false; error: string };

export function RequestActions({ requestId, tripId }: { requestId: string; tripId: string }) {
  ...
  return (
    <div className="mt-3">
      <div className="flex gap-2">
        <button
          disabled={pending}
          onClick={() => run(() => declineRequestAction(requestId, tripId))}
        ...
        <button
          disabled={pending}
          onClick={() => run(() => acceptRequestAction(requestId, tripId))}
