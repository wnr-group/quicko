...
export default async function AdminDashboard() {
  ...
  return (
    <>
      ...
      {admin && (
        <>
          <h2 className="mt-6 mb-2 text-[12px] font-bold uppercase tracking-wide text-muted">Operational metrics</h2>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <Link href="/admin/disputes" className="flex-1 rounded-2xl bg-white p-4 text-center font-semibold shadow-card active:scale-[0.99]">
          Disputes{openDisputes > 0 ? ` (${openDisputes})` : ""}
        </Link>
        <Link href="/admin/moderation" className="flex-1 rounded-2xl bg-white p-4 text-center font-semibold shadow-card active:scale-[0.99]">
          Moderation{openReports > 0 ? ` (${openReports})` : ""}
        </Link>
        <Link href="/support" className="flex-1 rounded-2xl bg-white p-4 text-center font-semibold shadow-card active:scale-[0.99]">
          Support{openSupport > 0 ? ` (${openSupport})` : ""}
        </Link>
        {admin && (
          <Link href="/admin/matching" className="flex-1 rounded-2xl bg-white p-4 text-center font-semibold shadow-card active:scale-[0.99]">
...
function RouteList({ title, rows, tone }: { title: string; rows: { route: string; n: number }[]; tone: "brand" | "ink" }) {
  return (
    <div className="rounded-2xl bg-white p-4 shadow-card">
      <div className="mb-2 text-[12px] font-bold uppercase tracking-wide text-muted">{title}</div>
      {rows.length === 0 ? (
        ...
        <ul className="flex flex-col gap-1.5">
          {rows.map((r) => (
            <li key={r.route} className="flex items-center justify-between text-[13px]">
              ...
              <span className={`ml-2 shrink-0 rounded-full px-2 py-0.5 text-[11px] font-bold ${tone === "brand" ? "bg-brand text-ink" : "bg-ink text-white"}`}>
                {r.n}
              </span>
            </li>
          ))}
